...
__global__ void matmul_shared_kernel(double *A, double *B, double *C, const int N)
{
    ...
    int indx = threadIdx.x + blockIdx.x * blockDim.x;   //Clacula index global
    int indy = threadIdx.y + blockIdx.y * blockDim.y;

    double sum = 0.0;
    for(int k=0;k < (N+ BLOCKSIZE-1)/BLOCKSIZE;k++){
        int rowA = k * BLOCKSIZE + threadIdx.x;         //Calcula index de la matriu A que li correspon al thread
        int colB = k * BLOCKSIZE + threadIdx.y;         //De la matriu B
        
        if(indy < N && rowA < N){
            A_tile[threadIdx.y][threadIdx.x] = A[indy*N + rowA];    //Posició dins de la matriu A ordenada row-major, per files
        }else{
            A_tile[threadIdx.y][threadIdx.x] = 0.0;
        }
        if(indx < N && colB < N){
            B_tile[threadIdx.y][threadIdx.x] = B[colB*N + indx];
        }else{
...
int main(int argc, char *argv[])
{
    ...
    int dimx = (N + BLOCKSIZE - 1) / BLOCKSIZE;
    int dimy = (N + BLOCKSIZE - 1) / BLOCKSIZE;
    dim3 threads_per_block(BLOCKSIZE, BLOCKSIZE);
    dim3 blocks_in_grid(dimx, dimy);
    CUDA_CHECK(cudaEventRecord(event_start));

    // TODO
    // Launch matmul_naive_kernel
    matmul_naive_kernel<<<blocks_in_grid, threads_per_block>>>(d_A, d_B, d_C, N);
    ...
    matmul_shared_kernel<<<blocks_in_grid, threads_per_block>>>(d_A, d_B, d_C, N);
