...
void integrateEuler(Particle *particles, const int N)
{
    ...
    for(int i=0;i<N;i++){
        ...
        double modul = sqrt(vx*vx + vy*vy + vz*vz);
        double val = K * modul * DT/M;
        particles[i].vel.x -= val * vx;
        particles[i].vel.y -= val * vy;
        particles[i].vel.z -= val * vz;
...
{
    #pragma acc parallel loop present(p_dst[0:N], p_src[0:N])
    for (int i = 0; i < N; ++i)
    {
    ...
    #pragma acc data copy(particles[0:N]) create(pFrame[0:N])
    {
        while (t <= TOTAL_TIME)
        {
            ...
            if (iter % FREQ == 0)
            {
                ...
                #pragma acc update self(pFrame[0:N])
                if (write_flag)
